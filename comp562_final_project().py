...
predicted_classes = np.column_stack([model.predict(X_test) for model in models])
...
for i in range(predicted_classes.shape[1]):
    actual_classes_col = y_test[:, i]
    predicted_classes_col = predicted_classes[:, i]

    c_matrix = confusion_matrix(actual_classes_col, predicted_classes_col)
